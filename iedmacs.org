...
Pour utiliser ~ellama~, il vous faudra tout d'abord installer
~ollama~.

En date du <2025-04-06 Sun>, l'installation est assez simple et peux
s'effectuer en 3 commandes :

- ~curl -fsSL https://ollama.com/install.sh | sh~ pour installer ollama
depuis le site officiel
- ~ollama --version~ pour confirmer la version installée
- ~ollama pull gemma3:4b~ pour installer gemma3 (plus petite version
  multimodale), mais il en existe beaucoup d'autres. (cf. le site
  officiel)

De cette manière le modèle installé 

Ensuite il faudra installer ~ellama~ avec ~M-x package-install
ellama~. Si la version sur ELPA est plus récente que celle indiqué sur
votre machine, il est possible que cela retourne une erreur durant
l'installation. Pour résoudre l'erreur, il faut rafraîchir le contenu
de emacs avec la commande ~M-x package-refresh-content~, puis relancer
emacs.

Enfin, dans le fichier init il faut ajouter le code suivant :
