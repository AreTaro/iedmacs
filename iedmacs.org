...
eww est le navigateur web d'Emacs. Très rudimentaire, il permet aussi
d'aller à l'essentiel, et d'éviter les nombreuses publicités. Quant
aux cookies ils sont pris en charge par le paquet URL
(cf. [[eww:https://www.gnu.org/software/emacs/manual/html_node/eww/Advanced.html][emacs/manual/eww/advanced]]).

~Ctrl-c Ctrl-o~ ouvrir un lien

~Alt-s Alt-w~ Chercher un mot ou une adresse

~r (eww-forward-url) ~ ou ~l (eww-back-url)~ pour reculer ou avancer
dans les pages viistées. Ces deux raccourcis ne fonctionnent qu'en
mode Emacs, et pas dans le mode evil~mode, sauf si définis ainis).
