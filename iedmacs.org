...
* Introduction à IEDmacs

IEDmacs est une version personnalisée d'Emacs, spécialement configurée
pour les étudiantes et étudiants de l'IED8 Paris. Si Emacs peut
sembler initimidant au premier abord et à mille lieux d'être considéré
comme le meilleur environnement de développement, son adaptabilité et
sa puissance m'ont toujours ramené à lui. 

A partir de cette expérience, j'ai cherché à rendre Emacs plus
accessible à ceux qui souhaiteraient l'explorer. Ce document est une
tentative de démystifier le code de configuration d'Emacs, contenu
dans le fichier ~init.el~, pour celles et ceux qui sont curieux
d'essayer. 

**La Programmation Lettrée**

Ce document adopte le concept de "[[https://fr.wikipedia.org/wiki/Programmation_lettr%C3%A9e][Progammation lettrée]]", un paradigme
proposé par Donald Knuth dans lequel le code et son explication sont
intégrés au sein d'un même document, pour une compréhension optimale. 

Dans ce contexte, Emacs et son extension Orgmode permettent de générer
un fichier de code à partir d'un fichier ~.org~, en extrayant
seulement les blocs de code indiqué par des balises spécifiques. Le
fichier ~init.el~ fourni dans ce dépôt a été généré automatiquement à
partir de ce fichier org, en utilisant le raccourci ~Ctrl-c Ctrl-v
Ctrl-t~.

**Structure du Document**

Le document se divise en plusieurs parties :

1. **Définitions** : Un lexique de termes et acronymes communs
   utilisés dans le document.
2. **Documentation Utilisateur** : Des instructions pour
   l'installation et l'utilisation d'IEDmacs.
3. **Documentation Système** : Une explication du fichier de
   configuration ~init.el~ et de comment il personnalise Emacs. 
4. **Recommandations** : Des conseils et des bonnes pratiques pour
   l'écriture et la maintenance de ce dépôt.
et le maintient de ce dépôt.

A travers cette structure, ce document espère faciliter l'installation
et l'utilisation d'IEDmacs, tout en introduisant les principes de base
de la programmation à ses utilisateurs.
...
*** Lancement de l'IEDmacs

Pour lancer IEDmacs, vous pouvez utiliser la commande ~emacs &~ dans
votre terminal. Cette commande démarre Emacs en arrière-plan, vous
permettant de continuer à utiliser le terminal pour d'autres
tâches. 

Alternativement, si une icône Emacs a été créée sur votre bureaau
après l'installation, vous pouvez lancer IEDmacs en double-cliquant
simplement sur cette icône.

Ces méthodes vous permettent de démarrer facilement et rapidement
votre environnement IEDmacs selon vos préférences.
...
azerty). Vous pouvez également écrire directement avec les balises
LaTex dans votre document ".org". Pour plus d'information sur la
synthaxe LaTex voir le dépôt de [[https://github.com/amartos/TexIEDP8][amartos]], ou sur le site du tuteur
informatique de l'ENS dédié au [[https://tuteurs.ens.fr/logiciels/latex/][LaTex]].
...
document PDF seront créés automatiquement.
...
clavier anglais. Le clavier anglais est par défaut. Pour changer de
clavier il faut utiliser la commande ~C-\~ en mode ~input~.
...
- ~P~ pour ajuter à la taille de la page
- ~n~ page suivante
- ~p~ page précédente
...
- ~k~ fermer le pdf ("kill" probablement)
...
*Eww, le navigateur web d'Emacs*

Eww est un navigateur web minimaliste intégré à Emacs. Il permet
d'accéder à l'essentiel, sans distractions ni publicités
intempestives.

*Gestion des cookies*

La gestion des cookies est assurée par le paquet URL. Pour plus
d'informations, référez-vous à la documentation d'Emacs :

- [[eww:https://www.gnu.org/software/emacs/manual/html_node/eww/Advanced.html][Chapitre sur Eww dans le manuel d'Emacs]]

*Ouvrir un lien*

- ~Ctrl-c Ctrl-o~

*Rechercher un mot ou une adresse*

- ~Alt-s Alt-w~

*Naviguer dans l'historique*

- Pour avancer : ~r~ (eww-forward-url)
- Pour reculer : ~l~ (eww-back-url)

*Remarques :*

- Ces raccourcis ne fonctionnent qu'en mode Emacs. En mode evil, les
  raccourcis sont accessibles avec ~SPC e~.

*** shell

Accéder au terminal du shell est simple en utilisant la commande ~M-x
shell~. Ceci ouvrira le shell dans un nouveau buffer.

Tandis que ~eshell~ offre une émulation du shell implémentée
directement en Emacs Lisp, l'Iedmacs préfère utiliser le ~shell~.

~Alt-p~ commande précédente

~Alt-n~ commande suivante
...
**** *Hera Cyber* (Janvier 2025)
- *0.0.0*: optimisation de la vitesse de démarrage, gptel, Fichiers
  récents, clignotement du curseur, surlignage de la ligne du curseur,
  localisation de la sauvegarde des fichiers, condition de la page
  d'ouverture.
- *0.2.0*: raccourcis pour eww en evil mode

**** *Izanagi Groovy* (Avril 2025)
- *0.0.0*: Modification du fichier latex pour avoir la table des
  matières en français, ajout du paquet ~ellama~ pour utiliser un LLM
  en local,
...
Lors du démarrage, Emacs recherche un fichier nommé "init.el"
qui contient du code Lisp a exécuter. L'IEDmacs génère automatiquement
ce fichier "init.el" à partir de blocs de code spécifiques. 

Chaque bloc de code commançant par la directive ~:tangle
"init.el"~ sera inclus dans le fichier "init.el" généré. Vous pouvez
le faire en utilisant la commande suivante : ~Ctrl-c Ctrl-v Ctrl-t~.

**Personnalisation de l'IEDmacs**

Pour personnaliser votre configuration dans IEDmacs, vous pouvez
ajouter ou supprimer des blocs de code selon vos besoins. 

Il est possible d'activer ou de désactiver les commandes. Seules les
commandes sans symboles ~;;~ devant seront exécutées.

Ainsi, pour désactiver une commande, ajoutez simplement ~;;~ devant la
ligne de la commande, ou bien effacez la directive "tangle" en tête de
bloc. Pour l'activer, retirer les symboles ~;;~.

Cela offre une flexibilité pour activer ou désactiver chaque
fonctionnalité selon vos besoins sans avoir à supprimer des blocs de
code entiers.

Voici la description de chaque bloc de code :

*** Entête du fichier
La convention veut que les informations de version soient indiquées au
début du fichier ~init.el~. De cette façon, si plusieurs versions sont
utilisées, il est possible d'identifier rapidement la version actuelle
du fichier ~init.el~ en utilisant la commande ~head 10
~/.emacs.d/init.el~.

Voici le contenu de ce bloc d'entête :
...
Le bloc d'entetête du fichier indique la version de l'environnement de
développement (par exemple, Hera Cyber 0.0.0) et précise que la
licence est la GPLv3.
...
La fonction principale,~ace-window~ est faite pour remplacer
~other-windows~ en affectant à chaque fenêtre un court, et unique
label. Quant il n'y a que deux fenêtre présentes, ~other-window~ est
appelé (à mon que ~aw-dispatch-always soit fixer à ~non-nil~)
...
Ibuffer est un paquet par défaut dans emacs depuis la
version 22. Permet une visualisation plus commode des buffers en
table, à la façon de Dired pour les fichiers.
...
Configuration du répertoire par défaut, lors de la recherche de
fichier par exemple.
...
Solution trouvée sur [[https://stackoverflow.com/questions/2680389/how-to-remove-all-files-ending-with-made-by-emacs][Stackoverflow]], et modifiée avec une proposition
alternative du manuel d'Emacs, chapitre [[eww:https://www.gnu.org/software/emacs/manual/html_node/emacs/Backup.html][Backup]]. Ceci permet
d'invisibiliser les sauvegardes automatiques faites par Emacs, et de
les conserver présentes dans le dossier.
...
(insert "    " (propertize (abbreviate-file-name (file-name-directory file)) 'face 'shadow) "\n")))
#+end_src
...
Cette partie explique comment Iedmacs est configuré afin de déterminté
la page d'accueil qui s'ouvrira au démarrage.

Si vous indiquez un fichier lors du lancement d'Emacs, par exemple en
utilisant la commande ~emacs monFichier~, Iedmacs ouvrira le buffer
nommé "monFichier".

Si aucun fichier n'est indiqué lors du lancement d'Emacs,
l'application s'ouvrira directement sur la page de démarrage de
l'Iedmacs.

Voici le code lisp pour cela :
...
En intégrant ce code à votre fichier de configuration "init.el", Emacs
lancera automatiquement la page de démarrage d'Iedmacs si aucun
fichier n'est spécifié au démarrage.

*** Evil Mode

Evil (Extensible VIm Layer) est paquet permettant l'utilisation des
principaux raccourci de l'IDE VIM dans Emacs.

Pour plus de détail voir le dépôt [[https://github.com/emacs-evil/evil][GitHub]].
...
Par défault l'~evil-mode~ est inactif (~evil-mode 0~). Pour l'activer
automatiquement, fixer le paramètre à ~1~.
...
À l'instar des systèmes d'exploitation, les grands types de modèles de
langage s'articulent autour de deux catégories principales : d'un
côté, les versions propriétaires, et de l'autre, leurs homologues
libres ou open-source.

Nous vous offrons un choix pour chacune de ces catégories : ~ellama~
pour les modèles ouverts et accessibles à tous, et ~gptel~ pour ceux
qui, plus fermés, exigent un accès réglementé.

En règle générale, les modèles open-source peuvent être installés
directement sur votre machine, sans besoin de connexion internet, bien
que leur taille soit conditionnée par les capacités de votre
ordinateur. Les modèles propriétaires, quant à eux, requièrent la
possession d'une clé d'accès, que l'on peut obtenir gratuitement dans
un premier temps, mais qui exige une inscription auprès des
fournisseurs, d'où une utilisation ultérieure souvent soumise à une
tarification.

**** ellama

Pour plonger dans l'univers d'~ellama~, la première étape consiste à
installer ~ollama~.

À la date du <2025-04-06>, l'opération d'installation se révèle d'une
simplicité déconcertante, ne nécessitant que trois commandes :

- ~curl -fsSL https://ollama.com/install.sh | sh~ pour récupérer
  ~ollama~ directement depuis le site officiel.
- ~ollama --version~ pour vérifier la version actuellement installée.
- ~ollama pull gemma3:4b~ pour ajouter à votre arsenal ~gemma3~, une
  version multimodale compacte, entre autres options disponibles
  (consultez le site officiel pour plus de détails).

Ainsi, votre modèle est prêt à s'animer. Il sera possible de
l'exécuter directement depuis le shell avec la commande ~ollama run
gemma3:4b~.

Ensuite, il vous faudra installer ~ellama~ à l'aide de la commande
~M-x package-install ellama~. Si la version sur ELPA s'avère plus
récente que celle présente sur votre machine, des erreurs pourraient
survenir durant l'installation. Pour remédier à cette situation,
rafraîchissez le contenu d'Emacs avec ~M-x package-refresh-content~,
puis relancez le programme.

Enfin, n'oubliez pas d'ajouter le code suivant dans votre fichier
init :
...
PlantUML est un outil open-source qui permet de créer des diagrammes à partir de texte.

Étant donné que l'utilisation de PlantUML dans Aremacs nécessite des
installations préalables, décrites ci-dessous, le paquet n'est pas
installé par défaut. Pour l'activer, il est nécessaire de procéder aux
installations depuis le terminal, d'installer le paquet dans Emacs,
puis d'ajouter le code suivant dans votre fichier /init.el/, en
incluant ~:tangle "init.el"~ comme pour les autres blocs.

*Installation depuis le Shell*

Pour utiliser /plantuml/ dans Emacs, il est essentiel d'avoir au
préalable installé les composants suivants :

- Java (exécutez la commande : ~sudo apt install default-jdk~),
- PlantUML (exécutez la commande : ~sudo apt install plantuml~),
- et Graphviz (exécutez la commande : ~sudo apt install graphviz~).

*Installation depuis Emacs*

Il est aussi nécessaire d'installer le paquet ~plantuml-mode~ (~M-x
package-install~) sur emacs.

*Configuration du fichier init.el*

Ensuite dans le fichier /init.el/, ajouter le code suivant. Pour
connaitre le chemin vers le fichier plantuml.jar (préalablement
téléchargé directement à partir du site ou avec la commande ~sudo apt
install plantuml~, utiliser la commande ~dpkg -L plantuml | grep '\.jar$'~.

#+begin_src emacs-lisp 
;; PLantUML
;; Définir le chemin du JAR pour Org Babel, même si on utilise l'exécutable
(setq org-plantuml-jar-path "/usr/share/plantuml/plantuml.jar")
(require 'plantuml-mode)

;; Si l'exécutable 'plantuml' est dans le PATH, ceci peut suffire
(setq plantuml-default-exec-mode :executable)

;; (Optionnel) Définir le type de sortie par défaut (png ou svg)
(setq plantuml-output-type "png")
#+end_src

Ensuite pour l'utiliser dans un document orgmode, executer le code
plantuml entre des balises _SRC avec ~C-c C-c~.

#+begin_src plantuml :file test-diagram.png
 @startuml
 Bob -> Alice : Hello
 Alice -> Bob : Hi!
 @enduml
...
Pour plus d'information sur le code plantuml:
- [[eww:https://usermanual.wiki/Pdf/PlantUMLLanguageReferenceGuideFR.1126508166/help][Guide de Référence]] (en français)
- [[eww:https://www.geeksforgeeks.org/unified-modeling-language-uml-introduction/][GeeksforGeeks]]  (en anglais)
...
Les sources principales qui m'ont permis d'avancer dans l'utilisation,
la configuration d'Emacs, mais également de sa philosophie sont par
ordre de découverte :
 
- Mike Zamansky, professeur d'université, simple et pédagogique.

- Protesilaos Stavrou (Prot), philosophe, détaillé et pragmatique.

- [[eww:https://systemcrafters.net/][System Crafters]], pour geek et niveau avancé.

Cette liste est tout à fait personnelle, et ne reflète absolument pas
le degré réel de connaissance et d'implication des personnes, même si
il semble que Prot soit très actif dans la communauté Emacs.
